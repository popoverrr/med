...
- **Лёгкий режим (`html.lite`)** — телефоны и планшеты (сенсор без мыши) или ширина < 1024 px; класс ставит `public/scripts/early.js` до первого кадра. В нём: секции всегда с непрозрачным фоном (фон не рисуется WebGL-слоем, который на телефоне отстаёт от нативного скролла), 3D-капля живёт только на первых экранах (hero, «Специалист», полоса доверия), а когда «Что это» закрывает экран, сцена паркуется (`html.scene-parked`: слой скрыт, кадры не запрашиваются); сцена рисуется через кадр (~30 fps) с dpr ≤ 1; фон документа переключается без scrub-интерполяции; шапка не прячется при скролле и без `backdrop-filter`. Pin-секции работают как раньше.
- **Порядок ScrollTrigger**: триггеры шапки/фона/появлений создаются раньше pin-секций, поэтому перед финальным `refresh()` вызывается `ScrollTrigger.sort()` — иначе тон шапки и фон переключались не на своих местах.
